...
exports.handler = async (event, context, callback) => {
  ...
  try {
    ...
    if (!qra_owner.identityId) await updateIdentityId(qra_owner, identityId);
  ...
  function updateIdentityId(qra_owner, identityId) {
    return new Promise(function(resolve, reject) {
      ...
      //***********************************************************
      conn.query(
        "UPDATE qras SET identityId = ?  WHERE idqras=?",
  ...
  function updateBio(bio, qra_owner) {
    return new Promise(function(resolve, reject) {
      ...
      //***********************************************************
      conn.query(
        "UPDATE qras SET bio = ?  WHERE idqras=?",
  ...
  function checkQraCognito(sub) {
    return new Promise(function(resolve, reject) {
      ...
      conn.query("SELECT * FROM qras where idcognito=? LIMIT 1", sub, function(
        err,
        info
      ) {
        // Call reject on error states, call resolve with results
  ...
  function saveActivity(qra_owner, datetime) {
    return new Promise(function(resolve, reject) {
      ...
      // ***********************************************************
      conn.query(
        "INSERT INTO qra_activities SET idqra = ?, activity_type='50', datetime=?",
  ...
  function insertNotifications(notifs) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      conn.query(
        "INSERT INTO qra_notifications (idqra_activity, idqra, qra, qra_avatarpic, datetime, " +
          "message, url, activity_type) VALUES ?",
        [notifs],
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
