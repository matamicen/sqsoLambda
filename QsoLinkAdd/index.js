...
exports.handler = async (event, context, callback) => {
  ...
  async function addQSOlinks(idqso, qsos_rel, qra_owner, datetime, qso) {
    ...
    for (let i = 0; i < qsos_rel.length; i++) {
      ...
      for (let j = 0; j < stakeholders.length; j++) {
        if (
          !qrasAll.some(elem => elem.idqra === stakeholders[j].idqra) &&
          stakeholders[i].idqra !== qra_owner.idqras
        ) {
          notif.push([
  ...
  function checkQSOlink(qso, idqso_rel) {
    return new Promise(function(resolve, reject) {
      ...
      conn.query(
        "SELECT * FROM qsos_links where idqso=? and idqso_Rel=?",
        [qso, idqso_rel],
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
  ...
  function addQSOlink(qso, idqso_rel) {
    return new Promise(function(resolve, reject) {
      ...
      conn.query(
        "INSERT INTO qsos_links SET idqso=?, idqso_rel=?, datetime=NOW()",
        [qso, idqso_rel],
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
  ...
  function UpdateLinksInQso(qso, counter) {
    return new Promise(function(resolve, reject) {
      ...
      conn.query(
        "UPDATE sqso.qsos SET links_counter = links_counter+? WHERE idqsos=?",
        [counter, qso],
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
  ...
  async function createNotifications(
    ...
    for (let i = 0; i < qras.length; i++) {
      if (
        !qrasAll.some(elem => elem.idqra === qras[i].idqra) &&
        ...
        if (qso_rel_qra === qras.qra)
          ...
          message =
            qra_owner.qra +
            " linked a QSO with another created by " +
            qso_rel_qra;
