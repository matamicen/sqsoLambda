...
exports.handler = async (event, context, callback) => {
  ...
  try {
    ...
    const { bio, idcognito, identityId, ...noBio } = qra;
    result.qra = noBio;
    if (!result.qra) {
    ...
    result.qra = { ...result.qra, account_type };
  ...
  function getAccountInfo(qra) {
    return new Promise(function(resolve, reject) {
      ...
      conn.query(
        "SELECT * from account_types where idaccount_types = ?",
        qra.account_type,
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
  ...
  function getNotifications(qra) {
    return new Promise(function(resolve, reject) {
      ...
      conn.query(
        "SELECT n.idqra_notifications, n.activity_type, n.idqra_activity, n.QRA, n.message, n.REF_QRA, n.QSO_GUID,  q.avatarpic as qra_avatarpic " +
  ...
  function updateLastLoginInQra(idqra) {
    return new Promise(function(resolve, reject) {
      ...
      // ***********************************************************
      conn.query(
        "UPDATE sqso.qras SET last_login = NOW()  WHERE idqras=?",
        idqra,
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
