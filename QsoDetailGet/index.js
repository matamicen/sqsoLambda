...
exports.handler = async (event, context, callback) => {
  ...
  try {
    ...
    if (event.context.sub) {
      qso.monthly_qso_views =
        (await getMonthlyViewsCounter(event.context.sub)) + 1;
  ...
  async function getQso(guid) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      conn.query("CALL `qso-detail-get`(?)", guid, function(err, info) {
        ...
        if (JSON.parse(JSON.stringify(info))[0].length > 0) {
  ...
  function getMonthlyViewsCounter(sub) {
    return new Promise(function(resolve, reject) {
      ...
      // from Congito ID");
      conn.query(
        "SELECT monthly_qso_views FROM qras where idcognito=?",
        sub,
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
  ...
  function updateMonthlyViewsCounterInQra(sub) {
    return new Promise(function(resolve, reject) {
      ...
      // ***********************************************************
      conn.query(
        "UPDATE sqso.qras SET monthly_qso_views = monthly_qso_views+1  WHERE idcognito=?",
        sub,
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
  ...
  function updateViewsCounterInQsos(idqsos) {
    return new Promise(function(resolve, reject) {
      ...
      // ***********************************************************
      conn.query(
        "UPDATE sqso.qsos SET views_counter = views_counter+1  WHERE idqsos=?",
        idqsos,
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
