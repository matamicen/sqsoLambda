...
exports.handler = async (event, context, callback) => {
  ...
  function getMonthlyViewsCounter(sub) {
    return new Promise(function(resolve, reject) {
      ...
      // from Congito ID");
      conn.query(
        "SELECT monthly_qra_views FROM qras where idcognito=?",
        sub,
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
  ...
  function updateMonthlyViewsCounterInQra(sub) {
    return new Promise(function(resolve, reject) {
      ...
      // ***********************************************************
      conn.query(
        "UPDATE sqso.qras SET monthly_qra_views = monthly_qra_views+1  WHERE idcognito=?",
        sub,
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
  ...
  function updateViewsCounterInQra(idqras) {
    return new Promise(function(resolve, reject) {
      ...
      // ***********************************************************
      conn.query(
        "UPDATE sqso.qras SET views_counter = views_counter+1  WHERE idqras=?",
        idqras,
        function(err, info) {
          // Call reject on error states, call resolve with results
          if (err) {
