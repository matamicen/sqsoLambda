            ...
            conn.query('SELECT * from qsos_likes WHERE idqso = ?', qso, function (error, qsoLikes) {
                ...
                if (error) {
                    ...
                    if (picked) {
                        ...
                        conn.query('DELETE FROM qsos_likes where idqso=? and idqra=?', [qso, idqra_owner], function (error, info) {
                            if (error) {
                                console.log("Error when Deleting QSO LIKES");
                                console.log(error.message);
                                conn.destroy();
                                response.body.error = 400;
                                response.body.message = "Error when Deleting QSO LIKES";
                                //return context.fail( "Error when Insert QSO LIKES");
                                return callback(null, response);
                            } else if (info.affectedRows > 0) {
                                console.log("QSOLIKES deleted ", info.affectedRows);
                                conn.destroy();
                                response.body.error = 0;
                                response.body.message = qsoLikes.length - 1;
                                console.log("Like Removed" + response.body.message);
                                return callback(null, response);
                            }
                        }); //End Delete
                    }
                        ...
                        conn.destroy();
                        response.body.error = 400;
                        response.body.message = "No Likes Found";
                        //return context.fail( "Error when Insert QSO LIKES");
                        return callback(null, response);
